...
- **Отдельная схема**:
  ...
  - **Минусы**: может стать сложнее при увеличении числа арендаторов.
...
- **Отдельная схема**:
  - *Риски*: Неправильная конфигурация может привести к утечке данных между арендаторами.
  - *Подход*: Хороший компромисс между изоляцией и управляемостью.

- **Таблица с идентификаторами**:
  - *Риски*: Высокая вероятность утечек данных между арендаторами, сложная логика контроля доступа.
...
В рамках Е2Е-теста идем по такому порядку:

1. **Тестирование через API (CRUD операции)**:
   - Проверка создания нового поста/постов.
   - Проверка редактирования существующего поста/постов.

Эти действия можно выполнить последовательно с помощью API-запросов.
